...
/*
$sqlDIDPattern="select did_pattern from vicidial_inbound_dids where did_description like '%equinox%' and (did_pattern like '44%' or  did_pattern like '20%')";
$rslt=mysql_query($sqlDIDPattern, $link);

while($row=mysql_fetch_assoc($rslt)){
    echo $group[]=$row['did_pattern'];
}*/
...
if ($DID=='Y')
{
	echo $stmt="select did_id from vicidial_inbound_dids where did_pattern IN($group_SQL);";
	$rslt=mysql_query($stmt, $link);
	if ($DB) {echo "$stmt\n";}
...
if ($DID=='Y')
	{
	echo $stmt="select count(*) from live_inbound_log where start_time >= '$query_date_BEGIN' and start_time <= '$query_date_END' and uniqueid IN($unid_SQL);";
	}
$rslt=mysql_query($stmt, $link);
if ($DB) {echo "$stmt\n";}
$rowx=mysql_fetch_row($rslt);
