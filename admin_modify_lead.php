...
#		echo "<a href=\"$PHP_SELF\">Close this window</a>\n<BR><BR>\n";
	...
	if ($lead_id != 'NEW') 
		...
		### find out if status(dispo) is a scheduled callback status
		$scheduled_callback='';
		echo $stmt="SELECT scheduled_callback from vicidial_statuses where status='$dispo';";
		$rslt=mysql_query($stmt, $link);
		$scb_count_to_print = mysql_num_rows($rslt);
		if ($scb_count_to_print > 0) 
			{
		...
		echo $stmt="SELECT scheduled_callback from vicidial_campaign_statuses where status='$dispo';";
		$rslt=mysql_query($stmt, $link);
		$scb_count_to_print = mysql_num_rows($rslt);
		if ($scb_count_to_print > 0) 
			{
		...
		echo $stmt="SELECT status,status_name,selectable,human_answered,category,sale,dnc,customer_contact,not_interested,unworkable from vicidial_statuses where selectable='Y' order by status";
		$rslt=mysql_query($stmt, $link);
